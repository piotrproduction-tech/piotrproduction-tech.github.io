...
export function createCityMetaEvolution({
    ...
    const proposal = {
      ...
      suggestion:
        state.awareness.level > 0.7
          ? "Miasto sugeruje dodanie nowego moduu introspekcji architektury."
          : "Miasto sugeruje uproszczenie czci workfloww.",
  ...
  function maybeApplyChange() {
    ...
    // Na razie: tylko zapisujemy, e mogoby co zmieni
    metaEvolution.appliedChanges.push({
      at: Date.now(),
      proposal,
      applied: false,
      note: "Zmiana architektury wymaga wiadomej decyzji architekta (Piotra)."
    });
  }
