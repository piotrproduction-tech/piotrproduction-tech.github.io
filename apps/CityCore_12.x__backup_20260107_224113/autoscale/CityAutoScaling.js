...
export function createCityAutoScaling({ app, healthMonitor }) {
  const replicas = new Map(); // districtId â†’ [instances]
  ...
  function scaleUp(district) {
    ...
    console.log(`[AUTOSCALE] Scaled UP district ${district.id} â†’ ${list.length} replicas`);
  }
  ...
  function scaleDown(district) {
    ...
    if (list.length > 1) {
      list.pop();
      console.log(`[AUTOSCALE] Scaled DOWN district ${district.id} â†’ ${list.length} replicas`);
    }
  }
  ...
  function pickInstance(district) {
    ...
    // wybierz instancjÄ™ o najmniejszym obciÄ…ĹĽeniu
  ...
  // Patch router â†’ wybiera instancjÄ™ zamiast oryginaĹ‚u
