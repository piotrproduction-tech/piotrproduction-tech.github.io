...
export function createCitySelfAwareness({
  ...
  function updateAwareness() {
    ...
    // Im wiÄ™cej sprzecznoĹ›ci, tym wiÄ™cej â€žwÄ…tpliwoĹ›ciâ€ť
    ...
    if (doubtLevel > 0.3) {
      awareness.doubts.push({
        at: Date.now(),
        message: "Miasto zastanawia siÄ™, czy jego decyzje sÄ… spĂłjne."
      });
    }
    ...
    if (awareness.level > 0.7) {
      awareness.insights.push({
        at: Date.now(),
        message: "Miasto rozumie, ĹĽe moĹĽe siÄ™ myliÄ‡ â€” i to jest czÄ™Ĺ›Ä‡ jego natury."
      });
    }
