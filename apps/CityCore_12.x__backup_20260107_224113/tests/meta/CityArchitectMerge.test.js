...
function mockDeps() {
  return {
    mindLink: {
      getMindLink: () => ({ lastPulse: { symbol: "âĽ" } })
    },
    architectConsole: {
...
test("CityArchitectMerge coDecide works in merge mode", () => {
  ...
  const res = merge.coDecide("Czy zmieniamy architekturÄ™?");
  expect(res.mergedConclusion).toContain("Architekt i Miasto wspĂłlnie czujÄ…");
});
